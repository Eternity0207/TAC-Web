...
export async function sendInvoiceEmail(order: Order, invoicePdf: Buffer): Promise<void> {
  ...
  const companyPhone = '+91 96641 61773';
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #4a7c23;">Your Invoice from ${companyName}</h2>
      <p>Dear ${order.customerName},</p>
      ...
      <div style="margin-top: 24px; color: #888; font-size: 0.85em;">
        <p>${companyName} - Royal Way to Stay Healthy</p>
      </div>
    </div>
  `;
  ...
  await transporter.sendMail({
    from: config.email.from,
    to: order.customerEmail,
    subject: `Invoice ${order.invoiceNumber} - ${companyName}`,
    html,
    attachments: [{
