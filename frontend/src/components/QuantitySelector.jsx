...
const QuantitySelector = ({ quantity, onChange, min = 1, max = 10, size = 'md', disabled = false }) => {
  const sz = size === 'sm' ? 'h-7 w-7 text-xs' : 'h-9 w-9 text-sm';
  const numW = size === 'sm' ? 'w-7' : 'w-9';

  return (
    <div className={`inline-flex items-center gap-0.5 rounded-full border border-gray-200 bg-white p-0.5 ${disabled ? 'opacity-60' : ''}`}>
      <motion.button
        type="button"
        whileTap={{ scale: 0.85 }}
